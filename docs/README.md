...
当前阶段：新增 `src/edgepick_bringup/README.md` 和 `src/edgepick_perception/README.md` 中的阶段 18 说明，记录 COCO 橘子 detector 与阶段 9 检测契约的对接方式。

完成内容：文档说明橘子检测继续复用 `TargetDetectionArray`，并通过 `detected_target_candidate_node` 进入 RGB-D 投影链路。
